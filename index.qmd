...
- Department of Informatics of the Brazilian Unified Health System ([DATASUS](https://datasus.saude.gov.br/)):
  - Annual population estimates by municipality, age, and sex for Brazil from 2000 to 2024 [@datasusb], used to calculate SISVAN's population coverage.
...
<!-- Nutritional status categories for weight-for-age, weight-for-height, stature-for-age, and BMI-for-age were assigned using z-scores calculated with the [`anthro`](https://cran.r-project.org/web/packages/anthro/index.html) R package [@dirkschumacher], following WHO child growth standards [@who2006]. Classifications were based on weight, height, age, and sex as described below:

| Indicator         | Category                    | z-Score Range      |
|-------------------|-----------------------------|--------------------|
| Weight-for-Age    | Severely underweight        | $z < -3$           |
|                   | Underweight                 | $-3 \leq z < -2$   |
|                   | Normal                      | $-2 \leq z \leq 1$ |
|                   | High                        | $z > 1$            |
| Weight-for-Height | Severely wasted             | $z < -3$           |
|                   | Wasted                      | $-3 \leq z < -2$   |
|                   | Normal                      | $-2 \leq z \leq 1$ |
|                   | Possible risk of overweight | $1 < z \leq 2$     |
|                   | Overweight                  | $2 < z \leq 3$     |
|                   | Obese                       | $z > 3$            |
| Stature-for-Age   | Severely stunted            | $z < -3$           |
|                   | Stunted                     | $-3 \leq z < -2$   |
|                   | Normal                      | $z \geq -2$        |
| BMI-for-Age       | Severely wasted             | $z < -3$           |
|                   | Wasted                      | $-3 \leq z < -2$   |
|                   | Normal                      | $-2 \leq z \leq 1$ |
|                   | Possible risk of overweight | $1 < z \leq 2$     |
|                   | Overweight                  | $2 < z \leq 3$     |
...
These categorizations align with the standards set by the World Health Organization (WHO) for assessing child growth and nutritional status [@who2008,  Section C]. -->
...
<!-- ::: {.callout-note}
See the [Data Categorization](#data-categorization) section for more information.
::: -->
...
# The correct approach would be to calculate nutritional status
# categories using age in days or months. However, since the SISVAN data only
# provide age in years, the resulting classification error is unacceptably high.
...
data <-
  ...
  mutate(
    ...
    weight_for_age = case_when(
      z_scores$zwei < -3 ~ "Severely underweight",
      (z_scores$zwei >= -3) & (z_scores$zwei < -2) ~ "Underweight",
      (z_scores$zwei >= -2) & (z_scores$zwei <= 1) ~ "Normal",
      z_scores$zwei > 1 ~ "High"
    ) |>
      factor(
        levels = c(
          "Severely underweight",
    ...
    weight_for_height = case_when(
      z_scores$zwfl < -3 ~ "Severely wasted",
      (z_scores$zwfl >= -3) & (z_scores$zwfl < -2) ~ "Wasted",
      (z_scores$zwfl >= -2) & (z_scores$zwfl <= 1) ~ "Normal",
      (z_scores$zwfl > 1) & (z_scores$zwfl <= 2) ~
        "Possible risk of overweight",
      (z_scores$zwfl > 2) & (z_scores$zwfl <= 3) ~ "Overweight",
      z_scores$zwfl > 3 ~ "Obese"
    ) |>
      factor(
        levels = c(
          "Severely wasted",
    ...
    height_for_age = case_when(
      z_scores$zlen < -3 ~ "Severely stunted",
      (z_scores$zlen >= -3) & (z_scores$zlen < -2) ~ "Stunted",
      z_scores$zlen >= -2 ~ "Normal"
    ) |>
      factor(
        levels = c(
          "Severely stunted",
    ...
    bmi_for_age = case_when(
      z_scores$zbmi < -3 ~ "Severely wasted",
      (z_scores$zbmi >= -3) & (z_scores$zbmi < -2) ~ "Wasted",
      (z_scores$zbmi >= -2) & (z_scores$zbmi <= 1) ~ "Normal",
      (z_scores$zbmi > 1) & (z_scores$zbmi <= 2) ~
        "Possible risk of overweight",
      (z_scores$zbmi > 2) & (z_scores$zbmi <= 3) ~ "Overweight",
      z_scores$zbmi > 3 ~ "Obese"
    ) |>
      factor(
        levels = c(
          "Severely wasted",
